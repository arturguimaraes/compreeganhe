...
class Helper {
	...
	public function loadPage($data) {
		...
		//Loads navbar
		if($data['page']['url'] != 'home' && $data['page']['url'] != 'login' && $data['page']['url'] != 'resetPassword')
	...
	public function sendEmail($to, $subject, $message) {
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL,"https://arturguimaraes.com/services/mail");
		curl_setopt($ch, CURLOPT_POST, 1);
		$subject = urlencode($subject);
		$message = urlencode($message);
		$from = urlencode("no-reply@compreeganhe.ne");
		$to = urlencode($to);
		curl_setopt($ch, CURLOPT_POSTFIELDS,
		            "subject=$subject&message=$message&from=$from&to=$to");
	...
	public function sendSignUpEmail($name, $cpf, $password, $email) {
		$to = $email;
	...
	public function sendChangePasswordEmail($user, $password) {
		...
		$message = "Prezado <b>$user->name</b>,<br><br>" .
				   "Você realizou a atualização de senha no Compre & Ganhe.<br>Seus dados:<br><br><br>" . 
				   "<b>Usuário:</b> $user->cpf<br>" . 
	...
	public function sendChangeResetPasswordEmail($user) {
		$to = $user->email;
		$subject = 'SENHA - COMPRE & GANHE: Recuperação de senha.';
		$message = "Prezado <b>$user->name</b>,<br><br>" .
				   "Você solicitou a recuperação de senha no Compre & Ganhe.<br><br>" . 
				   "<b>Usuário:</b> $user->cpf<br><br>" . 
				   "Clique no link a seguir para redefinir sua senha.<br><br>" .
				   "<b><a href='http://www.compreeganhe.net/resetPassword?us=$user->username&pw=$user->password'>Redefinir senha</a></b><br><br>" . 
				   "Este e-mail é confidencial! Não repasse essa senha para terceiros.<br><br>" .
				   "Atenciosamente,<br><br><a href='http://www.compreeganhe.net'>Compre & Ganhe</a>";
		return $this->sendEmail($to, $subject, $message);
	}
